...
public class RiskUIUtil {
	...
	static {
		...
		if (RiskUtil.streamOpener == null) {
			RiskUtil.streamOpener = new RiskIO() {
				...
				public InputStream openMapStream(String name)
						throws IOException {
					try {
						// TODO
						// TODO, do NOT even try this if we are inside a applet
						// sandbox
						// TODO or it will spam the logs with lots of: this
						// should never happen! bad file:...
						// TODO
						...
						} catch (Throwable ex) { // dont really care about this
													// one, it just means the
													// file is not found here
				...
				public ResourceBundle getResourceBundle(Class c, String n,
						Locale l) {
					return ResourceBundle.getBundle(c.getPackage().getName()
							+ "." + n, l);
				}
	 ...
	 * 
	 * @return boolean Return true if you open the online help, returns false
	 *         otherwise
	 */
	private static void riskOpenURL(URL docs) throws Exception {
		 ...
		 * if (applet == null ) {
		 * 
		 * File file = new File(docs); openURL(new URL("file://" +
		 ...
		 * 
		 * if ( os != null && os.startsWith("Windows")) { cmd =
		 * "rundll32 url.dll,FileProtocolHandler file://"+
		 * file.getAbsolutePath(); } else { cmd = "mozilla file://"+
		 * file.getAbsolutePath(); }
		 * 
		 * Runtime.getRuntime().exec(cmd);
		 * 
		 * return true; } catch(IOException x) { return false; }
		 */
	}
		...
		if (mapsdir == null) {
			...
			try {
				...
				if (checkForNoSandbox()) {
					...
					URL dir = RiskUIUtil.class.getResource("/maps");
	...
	public static Frame findParentFrame(Container c) {
		...
		return (Frame) javax.swing.SwingUtilities.getAncestorOfClass(
				Frame.class, c);

		/*
		 * // this does not work as using the method setContentPane makes this
		 * method return null
		 * 
		 * while(c != null) {
		 * 
		 ...
		 * 
		 * c = c.getParent(); } return (Frame)null;
		 */
	}
	...
	public static String getLoadFileName(Frame frame) {
		...
		if (webstart != null) {
			...
			try {
				...
				javax.jnlp.FileContents fc = fos.openFileDialog(SAVES_DIR,
			...
			int returnVal = fc.showDialog(frame, TranslationBundle.getBundle()
					.getString("mainmenu.loadgame.loadbutton"));
			if (returnVal == javax.swing.JFileChooser.APPROVE_OPTION) {
	...
	public static InputStream getLoadFileInputStream(String file)
		...
		if (webstart != null) {
			...
			javax.jnlp.FileContents fc = (javax.jnlp.FileContents) fileio
					.remove(file);
	...
	public static String getSaveFileName(Frame frame) {
		...
		if (webstart != null) {
			...
			JOptionPane.showMessageDialog(frame,
					"Please make sure to select a file name ending with \"."
							+ extension + "\"");
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				// Write your code here what to do if user has canceled Save
				// dialog
				return null;
			}
		}
	...
	public static void saveFile(String name, RiskGame obj) throws Exception {
		...
		if (webstart != null) {
			...
			javax.jnlp.FileContents fc = fss.saveFileDialog(
					name.substring(0, name.indexOf('/') + 1),
					new String[] { name.substring(name.indexOf('.') + 1) },
					stream,
	...
	public static void showAppletWarning(Frame frame) {
		...
		JOptionPane.showMessageDialog(frame, TranslationBundle.getBundle()
				.getString("core.error.applet"));
	...
	public static String getSystemInfoText() {
		...
		if (checkForNoSandbox()) {
			...
			// we CAN do this outside the sandbox, but for some reason it promps
			// the webstart
			try {
				netInfo = InetAddress.getLocalHost().getHostAddress() + " ("
		...
		return " " + RiskUtil.RISK_VERSION + " (save: " + RiskGame.SAVE_VERSION
				...
				+ System.getProperty("java.vm.version") + ", " + info + ") \n"
	...
	public static String getOSString() {
		String patch;
		if (checkForNoSandbox()) {
			patch = System.getProperty("sun.os.patch.level") + " ("
					+ System.getProperty("sun.arch.data.model") + "bit)";
		} else {
			patch = "?";
		}
		return System.getProperty("os.name") + " "
				+ System.getProperty("os.version") + " " + patch + " on "
	...
	public static void openAbout(Frame frame, String product, String version) {
		...
		int y = frame.getLocation().y + (frameSize.height - aboutSize.height)
				/ 2;
		if (x < 0)
			x = 0;
		if (y < 0)
	...
	public static void checkForUpdates(Risk risk) {
		...
		if (checkForNoSandbox()) {
			...
			try {
				...
				String[] newversion = (String[]) buffer
						.toArray(new String[buffer.size()]);

				if (newversion[0].startsWith("RISKOK ")) {

					String v = newversion[0].substring(7,
							newversion[0].length());
					...
					if (!v.equals(RiskUtil.RISK_VERSION)) {
						...
						v = resb.getString("mainmenu.new-version.text")
		...
		try {
			...
			if (lobbyURL != null) {
				...
				URLClassLoader ucl = URLClassLoader
						.newInstance(new URL[] { new URL("jar:" + lobbyURL
								+ "/LobbyClient.jar!/") });

				Class lobbyclass = ucl
						.loadClass("org.lobby.client.LobbyClientGUI");

				// TODO: should be like this
				// lobbyclass.newInstance();

				final javax.swing.JPanel panel = (javax.swing.JPanel) lobbyclass
						.getConstructor(new Class[] { URL.class }).newInstance(
								new Object[] { new URL(lobbyURL) });
	...
	private static File getFile(URL url) {
		...
		try {
			...
			// this is an attempt at a crazy workaround that should not really
			// work
			if (dir.startsWith("file://")) {
				md = new File(dir.substring(5, dir.length()).replaceAll(
						"\\%20", " "));
			} else {
				System.err
						.println("this should never happen! bad file: " + dir);
				md = new File("maps/");
			}

			// There is a bug in java 1.4/1.5 where it can not convert a URL
			// like
	...
	private static void setupLookAndFeel() {
		...
		/*
		 * OLD // set up system Look&Feel try {
		 * 
		 * String os = System.getProperty("os.name"); String jv =
		 * System.getProperty("java.version");
		 * 
		 ...
		 * ); } else {
		 * UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
		 * } } catch (Exception e) { RiskUtil.printStackTrace(e); }
		 */
	}
	...
	public static void parseArgs(String[] args) {
		...
		for (int nA = 0; nA < args.length; nA++) {
			...
			if (args[nA].length() > 5
					&& args[nA].substring(0, 5).equals("maps=")) {

				maps = args[nA].substring(5);
			}
			if (args[nA].length() > 6
					&& args[nA].substring(0, 6).equals("cards=")) {
	...
	public static void donate(Component parent) {
		...
		try {
			...
			JOptionPane.showMessageDialog(parent,
					"Unable to open web browser: " + e.getMessage(), "Error",
	...
	public static File getSaveGameDir() {
		...
		if (!userMaps.isDirectory() && !userMaps.mkdirs()) { // if it does not
																// exist and i
																// cant make it
	...
	public static double getDisplayDensity() {
		try {
			return ((Double) Class
					.forName("javax.microedition.midlet.ApplicationManager")
