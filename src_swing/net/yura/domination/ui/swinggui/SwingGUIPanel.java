...
public class SwingGUIPanel extends JPanel implements ActionListener
{
	...
	// these are needed here as they need to be changed to display different
	// amount of buttons
	...
	public SwingGUIPanel(Risk r)
	{
		...
		{
			RiskUtil.printStackTrace(th); // TranslationTool.jar could be
											// missing
		}
	...
	public void actionPerformed(ActionEvent a)
	{
		...
		{
			//System.out.println("playbutton pressed swingguipanel");
			if (editorTab.checkMap())
			{
				try
				{
					
					myrisk.newMemoryGame(editorTab.getMyMap(), editorTab.createMapFile());
					showMapImage(new ImageIcon(editorTab.getEditorPanel().getImagePic().getScaledInstance(203, 127,
	...
	class ConsoleTab extends JPanel implements SwingGUITab, ActionListener
	{
		...
		public void actionPerformed(ActionEvent a)
		{
			...
			else if (a.getActionCommand().equals("run script"))
			{
				...
				{
					// Write your code here what to do if user has canceled Open
					// dialog
				}
	...
	class GameTab extends JPanel implements SwingGUITab, ActionListener
	{
		...
		public GameTab()
		{
			...
			JLabel pixlogo = new JLabel("yura.net " + product + ", " + RiskUtil.GAME_NAME + " IDE",
					new javax.swing.ImageIcon(this.getClass().getResource("about.jpg")), JLabel.CENTER);
		...
		public void actionPerformed(ActionEvent a)
		{
			...
			else if ("continents".equals(actionCommand))
			{
				...
				JOptionPane.showMessageDialog(this, buffer.toString(), resbundle.getString("swing.button.continents"),
			...
			else if ("join game".equals(actionCommand))
			{
				...
				String result = JOptionPane.showInputDialog(RiskUIUtil.findParentFrame(this), "type the server name",
						myrisk.getRiskConfig("default.host"));
				if (result != null)
				{
					go("join " + result);
				}
			...
			else if ("options".equals(actionCommand))
			{
				...
				int result = JOptionPane.showOptionDialog(this, // the parent
																// that the
																// dialog blocks
				 ...
				 * true, myrisk); Dimension frameSize = frame.getSize();
				 * Dimension optionsSize = optionsDialog.getPreferredSize(); int
				 * x = frame.getLocation().x + (frameSize.width -
				 * optionsSize.width) / 2; int y = frame.getLocation().y +
				 * (frameSize.height - optionsSize.height) / 2; if (x < 0) x =
				 * 0; if (y < 0) y = 0; optionsDialog.setLocation(x, y);
		...
		public void getInput()
		{
			...
			if (localGame)
			{
				...
				if (myrisk.getGame().getState() != IRiskGame.STATE_DEFEND_YOURSELF)
				{
	...
	public void submitBug(String messageFromUser, String from, String subjectIn, String cause)
	{
		...
		String subject = RiskUtil.GAME_NAME + " " + RiskUtil.RISK_VERSION + " SwingGUI "
				+ TranslationBundle.getBundle().getLocale().toString() + " " + subjectIn;
		...
		try
		{
			if (RiskUIUtil.checkForNoSandbox())
			{
				...
				net.yura.grasshopper.BugSubmitter
						.submitBug(map, from, subject, cause, RiskUtil.GAME_NAME, RiskUtil.RISK_VERSION + " (save: "
								+ RiskGame.SAVE_VERSION + " network: " + IRiskGame.NETWORK_VERSION + ")",
		...
		{
		} // maybe Grasshopper.jar is missing

		// if for some reason we can not send with grasshopper, we fall back to
		// client email
		try
		{
			String text = (messageFromUser != null ? messageFromUser : "") + "\n\n\n" + debugTab.getDebugText() + "\n"
					+ debugTab.getErrText() + "\n\n" + "OS: " + RiskUIUtil.getOSString() + "\n" + "ID: "
					+ net.yura.lobby.mini.MiniLobbyClient.getMyUUID();

			// for some reason + does not get decoded, so we set it back to a
			// space
			URL url = new URL("mailto:yura@yura.net" + "?subject="
					+ URLEncoder.encode(subject, "UTF-8").replace('+', ' ') + "&body="
					+ URLEncoder.encode(text, "UTF-8").replace('+', ' '));
	...
	class DebugTab extends JSplitPane implements SwingGUITab, ActionListener
	{
		...
		public DebugTab()
		{
			...
			// debugScroll.setBorder( new javax.swing.border.TitledBorder(
			// goodBorder, "Debug Log",
			...
			// errScroll.setBorder( BorderFactory.createTitledBorder(
			// BorderFactory.createLoweredBevelBorder() , "Error Log",
			// javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
			// javax.swing.border.TitledBorder.ABOVE_TOP ) );
		...
		public void start()
		{
			...
			if (RiskUIUtil.checkForNoSandbox())
			{
				...
				try
				{
					...
					// HACK this will print any problems loading the Preferences
					// before we start grasshopper
					java.util.prefs.Preferences.userRoot(); // returns
	...
	public void go(String input)
	{
		...
		if (gameState != 2 || !myrisk.getGame().getSetupDone())
		{
			blockInput();
		}
	...
	public void Commands()
	{
		...
		try
		{
			...
			JOptionPane.showMessageDialog(RiskUIUtil.findParentFrame(this), commands,
					resbundle.getString("swing.message.commands"), JOptionPane.PLAIN_MESSAGE);
		}
		catch (Exception e)
		{
	...
	public void showQuestion(int n)
	{
		...
		String[] options = { resbundle.getString("swing.move.move"), resbundle.getString("swing.move.moveall"),
				resbundle.getString("swing.move.cancel") };

		int a = JOptionPane.showOptionDialog(RiskUIUtil.findParentFrame(this), // the
																				// parent
																				// that
																				// the
																				// dialog
				...
				resbundle.getString("swing.move.title"), // the title of the
															// dialog window
	...
	{
		JOptionPane.showMessageDialog(this, resbundle.getString("swing.message.error") + " " + error,
				resbundle.getString("swing.title.error"), JOptionPane.ERROR_MESSAGE);
	}

	public void showMission(String mission)
	{
		JOptionPane.showMessageDialog(this, resbundle.getString("swing.message.mission") + " " + mission,
				resbundle.getString("swing.title.mission"), JOptionPane.INFORMATION_MESSAGE);
	}
	...
	public void pprepaintCountries()
	{
		...
		if (tmp.equals(resbundle.getString("game.tabs.continents")))
		{
			newview = PicturePanel.VIEW_CONTINENTS;
		}
		else if (tmp.equals(resbundle.getString("game.tabs.ownership")))
		{
			newview = PicturePanel.VIEW_OWNERSHIP;
		}
		else if (tmp.equals(resbundle.getString("game.tabs.borderthreat")))
		{
			newview = PicturePanel.VIEW_BORDER_THREAT;
		}
		else if (tmp.equals(resbundle.getString("game.tabs.cardownership")))
		{
			newview = PicturePanel.VIEW_CARD_OWNERSHIP;
		}
		else if (tmp.equals(resbundle.getString("game.tabs.troopstrength")))
		{
			newview = PicturePanel.VIEW_TROOP_STRENGTH;
		}
		else if (tmp.equals(resbundle.getString("game.tabs.connectedempire")))
		{
			newview = PicturePanel.VIEW_CONNECTED_EMPIRE;
		}
	...
	/**
	 * called by the map editor to show a preview of the map we are
	 * making/testing
	 */
	...
	class SwingRiskAdapter extends RiskAdapter
	{
		 ...
		 * @param output
		 * @param redrawNeeded
		 *            If frame needs to be redrawn
		 * @param repaintNeeded
		 *            If frame needs to be repainted
		 */
		...
		public void needInput(int s)
		{
			...
			if (gameState != -1 && gameState != IRiskGame.STATE_NEW_GAME)
			{
				gameTab.getInput();
			}
			...
			else if (gameState == IRiskGame.STATE_TRADE_CARDS)
			{
			...
			else if (gameState == IRiskGame.STATE_ROLLING)
			{
				showDice(myrisk.getGame().getNoAttackDice(), true);
				inGameCards.show(inGameInput, "roll");
			}
			else if (gameState == IRiskGame.STATE_BATTLE_WON)
			{
			...
			else if (gameState == IRiskGame.STATE_DEFEND_YOURSELF)
			{
		 ...
		 * 
		 * @param state
		 *            The message that is needed to be displayed
		 */
		public void setGameStatus(String state)
		{
		 ...
		 * 
		 * public void openBattle(int c1num, int c2num) {} public void
		 * setNODAttacker(int n) {} public void setNODDefender(int n) {}
		 */
	...
	public JPanel makeGameOptionsPanel()
	{
		...
		mapViewComboBox.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent a)
			{
	...
	class GamePanel extends JPanel
	{
		...
		public GamePanel()
		{
			 ...
			 * java.awt.Insets(3, 3, 3, 3); c.fill = GridBagConstraints.BOTH;
			 * 
			 * c.gridx = 0; // col c.gridy = 0; // row c.gridwidth = 1; // width
			 * c.gridheight = 1; // height this.add(gameOptions, c);
			 * 
			 * c.gridx = 0; // col c.gridy = 1; // row c.gridwidth = 1; // width
			 * c.gridheight = 1; // height this.add(pp, c);
			 * 
			 * c.gridx = 0; // col c.gridy = 2; // row c.gridwidth = 1; // width
			 * c.gridheight = 1; // height this.add(inGameInput, c);
			 */
			JPanel ppBorder = new JPanel(new BorderLayout());
			ppBorder.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(5, 10, 5, 10),
		...
		public void mapClick(int[] countries, MouseEvent e)
		{
			...
			else if (gameState == IRiskGame.STATE_ATTACKING)
			{
				...
				{
					attacker.setText(resbundle.getString("game.note.attackerisseldefender").replaceAll("\\{0\\}",
							myrisk.getCountryName(countries[0])));
				}
				else
				{
	...
	class SetupPanel extends JPanel implements ActionListener
	{
		...
		public SetupPanel()
		{
			...
			playerOptions.setBorder(javax.swing.BorderFactory.createTitledBorder(resbundle
					.getString("newgame.label.players")));

			namedColors = new NamedColor[] { new NamedColor(Color.PINK, "pink", resbundle.getString("color.pink")),
			...
			final String[] names = { resbundle.getString("newgame.label.name"),
					resbundle.getString("newgame.label.color"), resbundle.getString("newgame.label.type"), };
			...
			/*
			 * data = new Object[][] { { name , namedColors[4],
			 * resbundle.getString("newgame.player.type.human")}, {"bob",
			 * namedColors[1],
			 * resbundle.getString("newgame.player.type.easyai")}, {"fred",
			 * namedColors[9],
			 * resbundle.getString("newgame.player.type.easyai")}, {"ted",
			 * namedColors[11],
			 * resbundle.getString("newgame.player.type.easyai")}, {"yura",
			 * namedColors[6],
			 * resbundle.getString("newgame.player.type.hardai")}, {"lala",
			 * namedColors[2],
			 * resbundle.getString("newgame.player.type.hardai")} };
			 */
			...
			{ // AbstractTableModel
				...
				public void setValueAt(Object aValue, int row, int column)
				{
					if (column == 0)
					{
						if (((String) aValue).length() > 15)
						{
							aValue = ((String) aValue).substring(0, 15);
						}
					}
					super.setValueAt(aValue, row, column);
				} // if(column==0 && ((String)aValue).indexOf('$')!=-1 ) {
					// return; } else { }
			};
			...
			newPlayer.addActionListener(new ActionListener()
			{
				public void actionPerformed(ActionEvent a)
				{
					...
					if (localGame)
					{
						...
						((DefaultTableModel) dataModel).addRow(new Object[] {
								resbundle.getString("newgame.newplayername"), namedColors[0], playerTypes[0] });
					...
					else
					{
						...
						// Options
						String[] options = { "OK", "Cancel" };
						int result = JOptionPane.showOptionDialog(RiskUIUtil.findParentFrame(SwingGUIPanel.this), // the
																													// parent
																													// that
																													// the
																													// dialog
																													// blocks
								message, // the dialog message array
								"create new player", // the title of the dialog
														// window
								JOptionPane.OK_CANCEL_OPTION, // option type
								JOptionPane.QUESTION_MESSAGE, // message type
								null, // optional icon, use null to use the
										// default icon
								options, // options string array, will be made
											// into buttons
						...
						if (result == 0)
						{
							...
							go("newplayer " + type + " " + ((NamedColor) colorComboBox.getSelectedItem()).getRealName()
									+ " " + ((JTextField) message[1]).getText());
			...
			defaultPlayers.addActionListener(new ActionListener()
			{
				public void actionPerformed(ActionEvent a)
				{
			...
			mapOptions
					.setBorder(javax.swing.BorderFactory.createTitledBorder(resbundle.getString("newgame.label.map")));
			...
			cardOptions.setBorder(javax.swing.BorderFactory.createTitledBorder(resbundle
					.getString("newgame.label.cards")));
			...
			chooseCards.addActionListener(new ActionListener()
			{
				public void actionPerformed(ActionEvent a)
				{
					...
					String name = RiskUIUtil.getNewFile(RiskUIUtil.findParentFrame(SwingGUIPanel.this),
							RiskFileFilter.RISK_CARDS_FILES);
			...
			GameTypeButtons.setBorder(javax.swing.BorderFactory.createTitledBorder(resbundle
					.getString("newgame.label.gametype")));

			JPanel cardsOptions = new JPanel();
			cardsOptions.setLayout(new javax.swing.BoxLayout(cardsOptions, javax.swing.BoxLayout.Y_AXIS));
			cardsOptions.setBorder(javax.swing.BorderFactory.createTitledBorder(resbundle
					.getString("newgame.label.cardsoptions")));
			...
			GameOptionsButtons.setBorder(javax.swing.BorderFactory.createTitledBorder(resbundle
					.getString("newgame.label.startgameoptions")));
			...
			startGame.addActionListener(new ActionListener()
			{
				public void actionPerformed(ActionEvent a)
				{
					...
					if (players.getRowCount() >= 2 && players.getRowCount() <= IRiskGame.MAX_PLAYERS)
					{
					...
					// if ((players.getRowCount() == 2) && (
					// !(((String)players.getValueAt(0, 2)).equals("Human")) ||
					// !(((String)players.getValueAt(1, 2)).equals("Human")) ||
					// !(domination.isSelected()) ) ) {
					// setupOK=false;
					...
					if (setupOK == true)
					{
						...
						go("startgame " + type + ((AutoPlaceAll.isSelected()) ? (" autoplaceall") : (""))
		...
		public void resetPlayers(boolean lastUsed)
		{
			...
			while (players.getRowCount() != 0)
				((DefaultTableModel) dataModel).removeRow(0);
			...
			for (int c = 1; c <= IRiskGame.MAX_PLAYERS; c++)
			{
				...
				if (!"".equals(name) && !"".equals(color) && !"".equals(type))
				{
					try
					{
						((DefaultTableModel) dataModel).addRow(new Object[] { name,
								findColor(ColorUtil.getColor(color)), findType(myrisk.getType(type)) });
					}
					catch (Exception ex)
					{
		...
		public NamedColor findColor(int color)
		{
			...
			for (int a = 0; a < namedColors.length; a++)
			{
				...
				if (namedColors[a].getRGB() == color)
				{
					return namedColors[a];
				}
	...
	class placeArmiesPanel extends JPanel
	{
		...
		public placeArmiesPanel()
		{
			...
			autoplace.addActionListener(new ActionListener()
			{
				public void actionPerformed(ActionEvent a)
				{
					go("autoplace");
	...
	class tacMovePanel extends JPanel
	{
		...
		public tacMovePanel()
		{
			...
			country1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory
					.createBevelBorder(javax.swing.border.BevelBorder.LOWERED), "Source",
					javax.swing.border.TitledBorder.LEADING, javax.swing.border.TitledBorder.TOP, new java.awt.Font(
							"SansSerif", 0, 11), new java.awt.Color(60, 60, 60)));
			...
			country2.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory
					.createBevelBorder(javax.swing.border.BevelBorder.LOWERED), "Target",
					javax.swing.border.TitledBorder.LEADING, javax.swing.border.TitledBorder.TOP, new java.awt.Font(
							"SansSerif", 0, 11), new java.awt.Color(60, 60, 60)));
			...
			moveButton.addActionListener(new ActionListener()
			{
				public void actionPerformed(ActionEvent a)
				{
					...
					if (!(country1.getText().equals("")) && !(country2.getText().equals("")))
					{
						...
						// FIXME: get the id of the country, don't use the
						// string name
						int nCountryId = myrisk.getGame().getCountryInt(pp.getC1()).getColor();
						// int nCountryId =
						// myrisk.getGame().getCountry(country1.getText()).getColor();
